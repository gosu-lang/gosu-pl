...
public class SQLPluginTest {
  ...
  public void getDdlType() {
    IType result = TypeSystem.getByFullNameIfValid("gosu.db.foo.Users");
    assertNotNull(result);
    assertEquals("gosu.db.foo.Users", result.getName());
  }

  @Test
  public void getSQLType() {
    IType result = TypeSystem.getByFullNameIfValid("gosu.db.foo.MyQuery");
    assertNotNull(result);
    assertEquals("gosu.db.foo.MyQuery", result.getName());
  }

  @Test
  public void getTablesFromSelect() {
    IType result = TypeSystem.getByFullNameIfValid("gosu.db.foo.MyQuery.Contact");
    assertNull(result);
  }

  @Test
  public void getTypeExplicitly() {
    IType result = TypeSystem.getByFullNameIfValid("gosu.db.foo.Users.Contact");
    assertNotNull(result);
    assertEquals("gosu.db.foo.Users.Contact", result.getName());
  }

  @Test
  public void getNamesFromSQL() {
    ISQLQueryType result = (ISQLQueryType) TypeSystem.getByFullNameIfValid("gosu.db.foo.MyQuery");
    assertNotNull(result);
    assertEquals("gosu.db.foo.MyQuery", result.getName());
    assertEquals("gosu.db.foo", result.getNamespace());
    assertEquals("MyQuery", result.getRelativeName());
  }

  @Test
  public void getNonExistantType() {
    IType result = TypeSystem.getByFullNameIfValid("gosu.db.foo.Unknown.DoesNotExist");
    assertNull(result);
  }
  ...
  public void testSQLResult() {
    TypeSystem.getByFullNameIfValid("gosu.db.foo.Users");
    ISQLQueryType result = (ISQLQueryType) TypeSystem.getByFullNameIfValid("gosu.db.foo.MyQuery2");
    assertNotNull(result);
    SQLBaseTypeInfo ti = (SQLBaseTypeInfo) result.getTypeInfo();
  ...
  public void oneSourceWithMultipleTypes() {
    IType result = TypeSystem.getByFullNameIfValid("gosu.db.foo.Vehicles.Car");
    assertNotNull(result);
    assertEquals("gosu.db.foo.Vehicles.Car", result.getName());

    result = TypeSystem.getByFullNameIfValid("gosu.db.foo.Vehicles.Motorcycle");
    assertNotNull(result);
    assertEquals("gosu.db.foo.Vehicles.Motorcycle", result.getName());
  }

  @Test
  public void getColumnDefs() {
    ISQLTableType result = (ISQLTableType) TypeSystem.getByFullNameIfValid("gosu.db.foo.Users.Contact");
    assertNotNull(result);
  ...
  public void getTypeInfo() {
    ...
    assertNotNull(result);
    assertEquals("gosu.db.foo.Users.Contact", result.getName());
    assertEquals("gosu.db.foo.Users", result.getNamespace());
  ...
  public void getMethodInfo() {
    ...
    assertEquals("gosu.db.foo.Users.Contact", findByAge.getReturnType().getName()); //returns single Contact

    IMethodInfo findAllByAge = ti.getMethod("findAllByAge", JavaTypes.pINT());
    assertNotNull(findAllByAge);
    assertEquals("java.lang.Iterable<gosu.db.foo.Users.Contact>", findAllByAge.getReturnType().getName()); //returns Iterable of Contacts //TODO KB
  ...
  public void getRawSQL() {
    ISQLDdlType result = (ISQLDdlType) TypeSystem.getByFullNameIfValid("gosu.db.foo.Users");
    assertNotNull(result);
    assertEquals("gosu.db.foo.Users", result.getName());
 ...
 // @Ignore("Fails individually on result != null, but succeeds in the whole test")
  public void getResultInfo() throws InterruptedException{
    TypeSystem.getByFullName("gosu.db.foo.Users");
    ISQLQueryType query = (ISQLQueryType)TypeSystem.getByFullName("gosu.db.foo.MyQuery2");
    query.getTypeInfo();
    ISQLQueryResultType expectedResult = query.getResultType();
    assertNotNull(expectedResult);
    ISQLQueryResultType result = (ISQLQueryResultType) TypeSystem.getByFullName("gosu.db.foo.MyQuery2Result");
    assertEquals(result.getTable().getTable().getTypeName(), "Contact");
  }

  @Test
  public void getSynthesizedMethod() {
    ISQLTableType result = (ISQLTableType) TypeSystem.getByFullNameIfValid("gosu.db.foo.Bars.Baz");
    assertNotNull(result);
    assertEquals("gosu.db.foo.Bars.Baz", result.getName());
    assertEquals("gosu.db.foo.Bars", result.getNamespace());
  ...
  public void domainLogicMustExtendSQLRecord() {
    ISQLTableType baseType = (ISQLTableType) TypeSystem.getByFullName("gosu.db.foo.BadExamples.Invalid");
    assertEquals("gosu.db.foo.BadExamples.Invalid", baseType.getName());
    assertEquals("gosu.db.foo.BadExamples", baseType.getNamespace());
    assertEquals("Invalid", baseType.getRelativeName());

    //assert the extension type exists ...
    IType extensionType = TypeSystem.getByFullName("gosu.db.foo.BadExampleExt.InvalidExt");
    assertEquals("gosu.db.foo.BadExampleExt.InvalidExt", extensionType.getName());
    assertEquals("gosu.db.foo.BadExampleExt", extensionType.getNamespace());
